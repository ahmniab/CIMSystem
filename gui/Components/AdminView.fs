...
module AdminView =
    ...
    let view (onGoToMovies: unit -> unit) (onGoToHalls: unit -> unit) =
        Component(fun ctx ->
            ...
            let endDateStr =
                ctx.useState (DateTime.Now.AddHours(2.0).ToString("yyyy-MM-dd HH:mm"))
            ...
            let handleSchedule () =
                if movies.Current.Length = 0 then
                    statusMsg.Set "Error: No movies available."
                else if physicalHalls.Current.Length = 0 then
                    statusMsg.Set "Error: No halls defined. Go to Manage Halls."
                else
                    ...
                    if isValidStart && isValidEnd && startT < endT then
                        ...
                        | Result.Ok _ ->
                            ...
                            refreshData ()
                        | Result.Error msg -> statusMsg.Set $"Error: {msg}"
                    else
                            ...
                            [ DockPanel.children
                                  [ StackPanel.create
                                        [ StackPanel.dock Dock.Right
                                          ...
                                          StackPanel.children
                                              [ Button.create
                                                    [ Button.content "🏗️ Manage Halls"
                                                      Button.background Brushes.Orange
                                                      Button.foreground Brushes.White
                                                      Button.onClick (fun _ -> onGoToHalls ()) ]
                                                Button.create
                                                    [ Button.content "🎬 Manage Films"
                                                      Button.background Brushes.DarkBlue
                                                      Button.foreground Brushes.White
                                                      Button.onClick (fun _ -> onGoToMovies ()) ] ] ]
                                    TextBlock.create
                              ...
                              Border.child (
                                  StackPanel.create
                                      [ StackPanel.spacing 15.0
                                        StackPanel.children
                                            [ TextBlock.create
                                                  [ TextBlock.text "Schedule a Movie"
                                                    TextBlock.fontWeight FontWeight.Bold ]
                                              ...
                                              if physicalHalls.Current.Length > 0 then
                                                  ComboBox.create
                                                      [ ComboBox.width 300.0
                                                        ...
                                                        ComboBox.itemTemplate (
                                                            DataTemplateView<PhysicalHall>.create
                                                                (fun (h: PhysicalHall) ->
                                                                    TextBlock.create
                                                                        [ TextBlock.text (
                                                                              sprintf
                                                  ...
                                                  TextBlock.create
                                                      [ TextBlock.text "⚠️ No Halls Defined"
                                                          ...
                                                          StackPanel.create
                                                              [ StackPanel.children
                                                                    [ TextBlock.create
                                                                      ...
                                                                      TextBox.create
                                                                          [ TextBox.text startDateStr.Current
                                                                            TextBox.onTextChanged startDateStr.Set ] ] ]
                                                          StackPanel.create
                                                              [ StackPanel.children
                                                                    [ TextBlock.create
                                                                          [ TextBlock.text "End (yyyy-MM-dd HH:mm)" ]
                                                                      TextBox.create
                                                                          [ TextBox.text endDateStr.Current
                                                                            TextBox.onTextChanged endDateStr.Set ] ] ] ] ]

                                              if not (String.IsNullOrWhiteSpace(statusMsg.Current)) then
                                                  TextBlock.create
                                                      [ TextBlock.text statusMsg.Current
                                                        TextBlock.foreground (
                                                            if statusMsg.Current.StartsWith("Success") then
                                                                Brushes.Green
                                                            else
                                                                Brushes.Red
                                                        )
                                              ...
                                              Button.create
                                                  [ Button.content "📅 Schedule Session"
                                                    Button.background Brushes.Green
                                                    Button.foreground Brushes.White
                                                    Button.onClick (fun _ -> handleSchedule ()) ] ] ]
                              ) ]

                        TextBlock.create
                            [ TextBlock.text "Active Schedules"
                              TextBlock.fontSize 18.0
                              TextBlock.fontWeight FontWeight.Bold ]
                        ScrollViewer.create
                            [ ScrollViewer.height 150.0
                                                                          ...
                                                                          [ Button.create
                                                                                [ Button.dock Dock.Right
                                                                                  ...
                                                                                  Button.onClick (fun _ ->
                                                                                      CinemaService.deleteSession s.Id
                                                                                      |> ignore
